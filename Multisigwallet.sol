...
contract MultisigWallet is Ownable, Destroyable {
    ...
    uint requestNumber;
    // map owners to TransferRequests by their ID's
    mapping(address => mapping(uint => bool)) approvals;
    //keep track of users balance
    mapping(address => uint) balance;
    //map transfers to their IDs
    ...
    function approveTransfer(uint _transferID) public ownlyOwners {
        ...
        require(
            msg.sender != transferToApprove.requestCreator,
            "you cannot approve your transfer that you created"
        );
        require(
            // ensure a signature is needed
            transferToApprove.approvalCount < requiredApprovals,
            "Transfer already approved"
        );
        require(
            approvals[msg.sender][transferToApprove.id] != true,
            "you have already approve transfer"
        );
        approvals[msg.sender][transferToApprove.id] = true;
        transferToApprove.approvalCount++;
        emit TransferApproved(transferToApprove.id);
        //if approvals required is reached automatically send the transfer
        if (transferToApprove.approvalCount == requiredApprovals) {
            sendTransfer(transferToApprove.id);
        }
    }
    ...
    function sendTransfer(uint _transferID) public ownlyOwners {
        ...
        require(
            msg.sender == transferToSend.requestCreator,
            "you can only send your own created transfers"
        );
        require(
            transferToSend.approvalCount == requiredApprovals,
            "Transfer must be approved by all approvals"
        );
        require(
